...
//		delta_Un_ = -limit;
	...
	pPIDCObj->delta_Un = (q15_t)delta_Un_;
	//printf("%d %d %d\r\n", delta_Un_, pPIDCObj->errorn_1, pPIDCObj->errorn_2);
	pPIDCObj->Un += pPIDCObj->delta_Un; 
	
	if(pPIDCObj->Un > pPIDCObj->limit2)
		pPIDCObj->Un = pPIDCObj->limit2;
	else if(pPIDCObj->Un < pPIDCObj->limit1)
		pPIDCObj->Un = pPIDCObj->limit1;
